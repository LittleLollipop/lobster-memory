...
**⚠️ 批量取数四戒（2026-09-08 / 09-11：四条都造成过「静默失真」——不报错，只是结论错）**：
...
3. **`impact` 的 B 组关键词有覆盖度闸门**（`--max-cover`，默认 0.6）：
   ...
   确需保留用 `--max-cover 1.0`。
4. 🔴 **正文分裂在两个字段**（2026-09-11）：`seed_semantic.py` 走 `graph_api.upsert_node` 的
   **`summary`** 形参，本工具（`upsert` / `append`）写 **`content`**。
   项目图库实测 **136/220（62%）的节点正文只存在 summary 里**。
   **读侧已统一**：`get` / `search` / `dump` 三处都走 `node_body()`（content 优先、回落 summary）。
   修前症状：`get` 看起来是空的、`search` 搜不到（实测搜一个词 0 条，内容其实在）、
   `dump` 备份缺 62% 的正文。`selftest` 56/56 PASS。
   **写侧刻意不统一，且不许「自动跟随」**（我先做错了一次）：
   · 写 `summary` 看似能保持单份，但 **seed 重跑会覆盖 `summary`**（`upsert_node` 的 props 里
     本就有这个 key，`setdefault` 不生效）⇒ 写进去的内容**静默丢失**；
   · 写 `content` **不丢**（seed 不碰 content），代价是同一节点两份正文 —— 由 `node_body` 兜底。
   取后者。`upsert` / `append` 命中这种节点时会打印一句 ⚠️ 提醒。
   🔴 **要改 seed 管理节点的正文，去改 seed 脚本，别在库里改。**
